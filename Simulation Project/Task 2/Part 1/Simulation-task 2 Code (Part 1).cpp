//This is Project 2 (Part-1) from Shashank Shekhar [200262327].
#include<iostream>
#include<fstream>
#include<queue>
#include<stdlib.h>
...
void printval(ofstream &outfile, float mc, float cla, float cls, int buffer, queue<double> clr){
...
pair<double, char> findmin(double cla, double cls, queue<double> &clr){
...
int main(int argc, char* argv[]){
	double mc, cla, inter_cla, cls, inter_cls, inter_clr, mc_end;
	...
	//Assign Machine clock to 0 and start with the cla = 2 and cls = -1 (indicating that the service time is unknown) and buffer = 0
	mc = 0; cla = 2; cls = -1; buffer = 0;
	...
	while(mc<=mc_end){
		...
		if(trace == 'a' && !buffer){ //meaning there is nothing to process and hence the cls is unknown
			...
			cla+=inter_cla; cls = mc+inter_cls; buffer+=1;
		}
		else if(trace == 'a'){
			//Check for buffer overflow here
			if(buffer<max_buffer){
				cla+=inter_cla; buffer+=1; 
			}
			else{
				cla+=inter_cla; clr.push(mc+inter_clr);
			}
		}
		else if(trace == 's'){
